...
export const cleanDefaultBusinessProfile: BusinessProfile = {
  ...
  voucherNumberingMode: 'SEPARATE',
  paymentReceiptPrefix: 'RCPT-',
  nextPaymentReceiptNumber: 1,
  paymentVoucherPrefix: 'PMT-',
  nextPaymentVoucherNumber: 1,
  contraVoucherPrefix: 'CNTR-',
  nextContraVoucherNumber: 1,
  voucherUnifiedPrefix: 'VCH-',
  nextUnifiedVoucherNumber: 1,
...
export const normalizeBusinessProfile = (profile?: Partial<BusinessProfile> | null): BusinessProfile => {
  ...
  return {
    ...
    voucherNumberingMode: profile.voucherNumberingMode === 'UNIFIED' ? 'UNIFIED' : 'SEPARATE',
    paymentReceiptPrefix: profile.paymentReceiptPrefix !== undefined ? profile.paymentReceiptPrefix : (base.paymentReceiptPrefix || 'RCPT-'),
    nextPaymentReceiptNumber: Math.max(1, parseInt(String(profile.nextPaymentReceiptNumber ?? base.nextPaymentReceiptNumber ?? 1), 10) || 1),
    paymentVoucherPrefix: profile.paymentVoucherPrefix !== undefined ? profile.paymentVoucherPrefix : (base.paymentVoucherPrefix || 'PMT-'),
    nextPaymentVoucherNumber: Math.max(1, parseInt(String(profile.nextPaymentVoucherNumber ?? base.nextPaymentVoucherNumber ?? 1), 10) || 1),
    contraVoucherPrefix: profile.contraVoucherPrefix !== undefined ? profile.contraVoucherPrefix : (base.contraVoucherPrefix || 'CNTR-'),
    nextContraVoucherNumber: Math.max(1, parseInt(String(profile.nextContraVoucherNumber ?? base.nextContraVoucherNumber ?? 1), 10) || 1),
    voucherUnifiedPrefix: profile.voucherUnifiedPrefix !== undefined ? profile.voucherUnifiedPrefix : (base.voucherUnifiedPrefix || 'VCH-'),
    nextUnifiedVoucherNumber: Math.max(1, parseInt(String(profile.nextUnifiedVoucherNumber ?? base.nextUnifiedVoucherNumber ?? 1), 10) || 1),
    ...
    themeColor: profile.themeColor || base.themeColor || 'indigo',
    itemLineSettings,
    bottomNavConfig,
