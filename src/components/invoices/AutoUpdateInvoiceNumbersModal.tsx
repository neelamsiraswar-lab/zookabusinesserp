...
export const AutoUpdateInvoiceNumbersModal: React.FC<AutoUpdateInvoiceNumbersModalProps> = ({
  ...
  return (
    <DesktopModal
      ...
      onClose={onClose}
      size="xl"
      title="Auto-Update Invoice Numbers from Starting Sequence"
      subtitle="Re-number all tax invoices in continuous chronological order starting from company starting number"
      badge={
        <span className="text-[10px] font-extrabold uppercase px-2 py-0.5 rounded-full bg-indigo-500/20 text-indigo-700 dark:text-indigo-300 border border-indigo-400/30">
          Company & System Settings
        </span>
      }
      icon={<Hash className="w-5 h-5" />}
      iconBgColor="bg-indigo-600/20 text-indigo-600 dark:text-indigo-300 border border-indigo-400/30"
      bodyClassName="space-y-5 text-xs"
      footer={
