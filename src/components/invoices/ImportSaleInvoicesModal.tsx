...
export const ImportSaleInvoicesModal: React.FC<ImportSaleInvoicesModalProps> = ({
  ...
  return (
    <DesktopModal
      ...
      onClose={onClose}
      size="3xl"
      title="Import Sale Invoices"
      subtitle="Bulk upload historical invoices, POS receipts, and tax bills with automated GST & customer ledger sync"
      ...
      footer={
        <div className="flex items-center justify-between w-full">
          <div className="flex items-center gap-2 text-xs text-slate-500 dark:text-slate-400">
            {activeStep === 'PREVIEW' ? (
              <span>
                Ready to commit: <strong className="text-slate-800 dark:text-slate-200">{groupedInvoices.length} invoices</strong> (Total: {formatCurrency(totalGrand, business.currencySymbol)})
              </span>
            ) : (
              <span>Step 1: Upload CSV/Excel or paste data directly</span>
            )}
          </div>
          ...
          <div className="flex items-center gap-3">
            <button
              type="button"
              onClick={onClose}
              className="px-4 py-2 text-xs font-bold text-slate-600 dark:text-slate-400 hover:text-slate-800 dark:hover:white transition-colors cursor-pointer"
            >
              Cancel
            </button>
    ...
    >
        ...
        <div className="flex items-center justify-between px-6 py-2.5 bg-slate-100/60 dark:bg-slate-800/50 border-b border-slate-200 dark:border-slate-800 text-xs shrink-0">
