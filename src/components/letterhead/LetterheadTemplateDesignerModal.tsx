...
export const LetterheadTemplateDesignerModal: React.FC<LetterheadTemplateDesignerModalProps> = ({
  ...
  return (
    <DesktopModal
      ...
      size="full"
      allowMaximize={true}
      title="Letterhead Design Studio"
      subtitle="Customize header layout, typography, accent branding, watermark & signatory footer"
      icon={<Palette className="w-5 h-5 text-indigo-400" />}
      iconBgColor="bg-indigo-600/20 border border-indigo-500/30"
      bodyClassName="overflow-hidden flex flex-col flex-1 p-0"
      footer={
        <div className="flex justify-end gap-2 w-full">
          <button
            type="button"
            onClick={onClose}
            className="px-3.5 py-2 text-xs font-semibold text-slate-400 hover:text-white hover:bg-slate-800 rounded-xl transition cursor-pointer"
          >
            Cancel
          </button>
          <button
            type="button"
            onClick={handleSave}
            className="px-4 py-2 rounded-xl text-xs font-semibold bg-indigo-600 hover:bg-indigo-500 text-white shadow-md shadow-indigo-900/30 flex items-center gap-1.5 transition-colors cursor-pointer"
          >
