...
export const AnnouncementDetailModal: React.FC<AnnouncementDetailModalProps> = ({
  ...
  const publishedDate = new Date(announcement.publishedAt || Date.now()).toLocaleDateString('en-IN', {
    day: 'numeric',
    month: 'short',
    year: 'numeric'
  });
  ...
  return (
    <DesktopModal
      ...
      subtitle={`Published on ${publishedDate}`}
      icon={<Icon className="w-5 h-5 text-indigo-600 dark:text-indigo-400" />}
      iconBgColor="bg-indigo-50 dark:bg-indigo-950/70"
      bodyClassName="p-5 sm:p-6 space-y-4 text-slate-700 dark:text-slate-300 text-sm leading-relaxed"
      footer={
