  ...
  return (
    <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200/80 dark:border-slate-800 shadow-xs overflow-hidden transition-all">
      ...
      <DesktopModal
        ...
        size="2xl"
        allowMaximize={true}
        title={`Monthly Statement & Report Card (${monthLabel})`}
        subtitle="Computer-generated executive statement and compliance summary"
        icon={<Printer className="w-5 h-5 text-indigo-600 dark:text-indigo-400" />}
        iconBgColor="bg-indigo-50 dark:bg-indigo-950/70"
        bodyClassName="p-6 overflow-y-auto space-y-6 text-slate-800 dark:text-slate-200 text-xs"
        footer={
          <div className="flex justify-end gap-2.5 w-full">
            <button
              type="button"
              onClick={() => setShowPrintModal(false)}
              className="px-4 py-2 text-xs font-semibold rounded-xl border border-slate-300 dark:border-slate-700 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors cursor-pointer text-slate-700 dark:text-slate-200"
            >
              Close
            </button>
            <button
      ...
      >
        <div id="monthly-report-printable-area" className="space-y-6">
              {/* Business Header */}
