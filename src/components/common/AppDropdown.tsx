...
export function AppDropdown<T = string>({
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    ...
    switch (e.key) {
      ...
      case 'Home': {
        e.preventDefault();
        const firstValid = filteredOptions.findIndex(o => !o.disabled);
            ...
            <div className="p-1 mb-1 border-b border-slate-100 dark:border-slate-800">
              <div className="relative flex items-center">
                ...
                <input
                  ...
                  className="w-full pl-8 pr-7 py-1.5 text-xs bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-slate-800 dark:text-slate-100 placeholder-slate-400 outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500/20"
                />
                {searchQuery && (
                  <button
                    type="button"
                    ...
                    className="absolute right-2 text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 p-0.5 rounded cursor-pointer"
                    title="Clear search"
                  >
                    <X className="w-3 h-3" />
                  </button>
