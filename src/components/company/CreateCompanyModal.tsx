...
export const CreateCompanyModal: React.FC<CreateCompanyModalProps> = ({
  ...
  return (
    <DesktopModal
      ...
      size="3xl"
      allowMaximize={true}
      title="Register New Company / Business"
      badge={
        <span className="text-[9px] sm:text-[10px] font-extrabold uppercase bg-purple-500/20 text-purple-700 dark:text-purple-300 px-2 py-0.5 rounded-full border border-purple-400/40">
          Super Admin
        </span>
      }
      subtitle="Multi-entity provisioning with isolated GSTIN, books & user roles"
      icon={<Crown className="w-5 h-5 text-amber-500" />}
      iconBgColor="bg-purple-50 dark:bg-purple-950/70"
      headerActions={
        <button
          type="button"
          onClick={handleFillDemo}
          className="px-2.5 py-1.5 sm:px-3 sm:py-1.5 bg-purple-500/10 hover:bg-purple-500/20 text-purple-700 dark:text-purple-300 text-[11px] sm:text-xs font-semibold rounded-xl border border-purple-400/30 transition-all flex items-center gap-1 cursor-pointer"
    ...
    >
      <form id="create-company-form" onSubmit={handleSubmit} className="p-4 sm:p-6 space-y-4 sm:space-y-6 flex-1 text-slate-800 dark:text-slate-200">
